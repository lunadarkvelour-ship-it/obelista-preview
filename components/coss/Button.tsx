...
/** coss ui Button — НЕ shim. Это исходник coss, скопированный сюда, чтобы
 *  новый код мог импортировать coss-нативную кнопку из `@/components/coss/`,
 *  а не путаться со старым шимом `@/components/ui/button`, который
 *  по-прежнему экспортирует RAC-варианты (`default|success|ghost|subtle|
 *  outline|destructive|link`).
 *
 *  Постепенный переход:
 *    1) новый код берёт Button/Input/… из `@/components/coss/`;
 *    2) на пересечении с шимом — рядом, без конфликта имён;
 *    3) когда старый шим опустеет, переезд в `components/ui/`.
 *
 *  Прямая копия из реестра coss (npx shadcn view @coss/button), зависимости
 *  те же (@base-ui/react, @/components/ui/spinner). Зависимость от шима
 *  spinner — потому что coss-кнопка сама подкладывает Spinner при loading.
 */
...
import { Spinner } from "@/components/coss";
