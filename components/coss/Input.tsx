/** coss ui Input — копия из реестра coss. API: <Input className size? type? ...rest>.
 *
 *  ОТЛИЧИЕ ОТ СТАРОГО ШИМА: coss-input обёрнут в <span data-slot="input-control">,
 *  и в DOM живёт вложенный <input data-slot="input">. Старый шим клал ref прямо
 *  на <input> без обёртки.
 *
 *  ВНИМАНИЕ. Naming вставляет макросы по ref, и в шиме они ложились на <input>;
 *  в coss — на <span> обёртку. Если потребитель рассчитывает на `ref.current` как
 *  на <input>, миграция ломает Naming. В нашей панели Naming получает ref через
 *  React.createRef (см. sections/Naming.tsx) и не читает .current, так что для
 *  нас это не страшно; но это грабля, о которой стоит помнить.
 */
"use client";
